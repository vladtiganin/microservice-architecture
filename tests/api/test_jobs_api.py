...
async def test_get_jobs_returns_200_and_items(simple_async_client: httpx.AsyncClient):
    service_mock = Mock()
    service_mock.get_jobs = AsyncMock(return_value={
        "items": [
            {
                "id": 1,
                "type": "1",
...
@pytest.mark.asyncio
async def test_post_jobs_returns_200_and_created_job(simple_async_client: httpx.AsyncClient):
...
async def test_post_jobs_returns_500_when_service_raises_http_exception(simple_async_client: httpx.AsyncClient):
    service_mock = Mock()
    service_mock.create_job = AsyncMock(side_effect = HTTPException(status_code=500))
    app.dependency_overrides[dependencies.create_job_service_instance] = lambda: service_mock
...
async def test_get_job_by_id_returns_200_when_job_exists(simple_async_client: httpx.AsyncClient ):
    service_mock = Mock()
    service_mock.get_job_by_id = AsyncMock(return_value={
        "id": 1,
        "type": "1",
        "status": "pending",
        "payload": "1",
...
async def test_get_job_by_id_returns_200_when_job_exists(simple_async_client: httpx.AsyncClient ):
    service_mock = Mock()
    service_mock.get_job_by_id = AsyncMock(side_effect = HTTPException(404))
    app.dependency_overrides[dependencies.create_job_service_instance] = lambda: service_mock
